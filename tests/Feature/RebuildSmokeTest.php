...
class RebuildSmokeTest extends TestCase
{
    ...
    public function test_coach_can_update_program_and_maintain_sessions(): void
    {
        ...
        $component = Livewire::actingAs($coach)
            ...
            ->set('programForm.title', 'Updated Plan')
            ->set('programForm.goal', 'Return to sprinting')
            ->set('programForm.status', 'active')
            ->call('updateProgram')
            ->call('startEditSession', $session->id)
            ->set('editSessionForm.title', 'Updated Session')
            ->set('editSessionForm.focus', 'Strength')
            ->set('editSessionForm.scheduledOn', now()->addDay()->toDateString())
            ->set('editSessionForm.exercises.0.name', 'Trap bar deadlift')
            ->set('editSessionForm.exercises.0.sets', '4')
            ->set('editSessionForm.exercises.0.reps', '5')
            ->set('editSessionForm.exercises.0.media_url', 'https://example.com/trap-bar-demo.mp4')
            ->call('updateSession')
            ->assertHasNoErrors();
