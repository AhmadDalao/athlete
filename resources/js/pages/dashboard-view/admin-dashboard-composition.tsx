        ...
        <section id="admin-board" className="space-y-6">
            ...
            <div className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
                ...
                >
                    <WorkspaceTable minWidth="min-w-[720px]">
                        <WorkspaceTableHeader labels={['User', 'Role', 'Joined', 'Membership', 'Profile']} />
                        {admin.recentUsers.length === 0 ? (
                            <WorkspaceTableEmpty message="No users are available yet." colSpan={5} />
                        ) : (
                ...
                >
                    <WorkspaceTable minWidth="min-w-[680px]">
                        <WorkspaceTableHeader labels={['Athlete', 'Coach', 'Activity', 'Subscription', 'Profile']} />
                        {admin.athleteTable.length === 0 ? (
                            <WorkspaceTableEmpty message="No athletes are available yet." colSpan={5} />
                        ) : (
                            <tbody className="divide-y divide-stone-200">
            ...
            >
                <WorkspaceTable minWidth="min-w-[920px]">
                    <WorkspaceTableHeader labels={['Client', 'Plan', 'Status', 'Subscribed', 'Renewal', 'Value', 'Profile']} />
                    {admin.subscriptionTable.length === 0 ? (
                        <WorkspaceTableEmpty message="No subscriptions are available yet." colSpan={7} />
                    ) : (
                        <tbody className="divide-y divide-stone-200">
            ...
            <div className="space-y-4">
                ...
                >
                    <WorkspaceTable minWidth="min-w-[920px]">
                        <WorkspaceTableHeader labels={['Queue', 'User', 'Status', 'Date / timing', 'Value / provider', 'Details']} />
                        {admin.expiringMembershipWatchlist.length === 0 &&
                        admin.paymentAttentionQueue.length === 0 &&
                        admin.deviceAttentionQueue.length === 0 ? (
                            <WorkspaceTableEmpty message="No urgent operations records are queued right now." colSpan={6} />
                        ) : (
                            <tbody className="divide-y divide-stone-200">
                                {admin.expiringMembershipWatchlist.map((entry) => (
                                    <tr key={`renewal-${entry.userName}-${entry.planName}`} className="align-top transition-colors hover:bg-stone-50/80">
                                        <td className="px-4 py-4">
                                ...
                                {admin.paymentAttentionQueue.map((entry) => (
                                    <tr
                                        key={`payment-${entry.userName}-${entry.reference ?? entry.eventAt ?? entry.planName}`}
                                ...
                                {admin.deviceAttentionQueue.map((entry) => (
                                    <tr key={`device-${entry.userName}-${entry.provider}`} className="align-top transition-colors hover:bg-stone-50/80">
                                        <td className="px-4 py-4">
                ...
                <div className="grid gap-4 xl:grid-cols-[1.05fr_0.95fr]">
                    ...
                    >
                        <WorkspaceTable minWidth="min-w-[560px]">
                            <WorkspaceTableHeader labels={['Method', 'State', 'Accounts']} />
                            <tbody className="divide-y divide-stone-200">
                                {admin.signupMix.map((entry) => (
                                    <tr key={entry.method} className="transition-colors hover:bg-stone-50/80">
                    ...
                    >
                        <WorkspaceTable minWidth="min-w-[560px]">
                            <WorkspaceTableHeader labels={['Metric', 'Value', 'Meaning']} />
                            <tbody className="divide-y divide-stone-200">
