<div>
    ...
    <div class="tl-panel">
        <div class="tl-table-wrap"><table class="table tl-table align-middle">
            <thead><tr><th>Athlete</th><th>Status</th><th>Expires</th><th>Accept link</th><th>Action</th></tr></thead>
            <tbody>
            @forelse($invitations as $invite)
                <tr><td><strong>{{ $invite->name ?: 'No name' }}</strong><br><span class="tl-muted">{{ $invite->email }}</span></td><td><span class="tl-badge {{ $invite->status === 'pending' ? 'gold' : 'gray' }}">{{ $invite->status }}</span></td><td>{{ $invite->expires_at->format('Y-m-d') }}</td><td><code>{{ route('invites.accept', $invite->token) }}</code></td><td>@if($invite->status === 'pending')<button class="btn btn-outline-danger btn-sm" wire:click="cancel({{ $invite->id }})">Cancel</button>@endif</td></tr>
            @empty
